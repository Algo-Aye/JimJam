let audio1 = new Audio(
  ...
  function addAdminResponse(advice) {
  ...
  function sendPostRequest(chat_text) {
    // Create an XMLHttpRequest object
    var xhr = new XMLHttpRequest();
  
    // Prepare the URL
    var url = "/Restuarant/chatter.php"
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            addAdminResponse(xhr.responseText);
        }
    };
  
    // Prepare the JSON data
    var data = JSON.stringify({ message: chat_text });
